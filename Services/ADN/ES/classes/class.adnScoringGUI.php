...
class adnScoringGUI
{
    ...
    public function __construct()
    {
        ...
        $this->ctrl->saveParameter($this, array("sc_id"));
    ...
    protected function editScoring(ilPropertyFormGUI $a_form = null)
    {
        ...
        $this->tpl->setContent($a_form->getHTML() . "<br/>" .
    ...
    protected function initScoringForm()
    {
        ...
        if (adnSubjectArea::hasCasePart($event->getType())) {
            $case = new ilNumberInputGUI($this->lng->txt("adn_scoring_case"), "score_case");
            $case->setDecimals(1);
        ...
        // result mc
        include_once("./Services/ADN/EP/classes/class.adnAssignment.php");
        $mc_result = new ilSelectInputGUI($this->lng->txt("adn_result_mc"), "result_mc");
        ...
        // result case
        if (adnSubjectArea::hasCasePart($event->getType())) {
            $case_result = new ilSelectInputGUI($this->lng->txt("adn_result_case"), "result_case");
        ...
        $holdback_toggle = new ilCheckboxInputGUI($this->lng->txt("adn_holdback"), "blocked");
        $form->addItem($holdback_toggle);

        $holdback = new ilDateTimeInputGUI($this->lng->txt("adn_holdback_until"), "blocked_until");
        ...
        $holdback_by = new ilSelectInputGUI($this->lng->txt("adn_holdback_by"), "blocked_by");
    ...
    protected function saveScoring()
    {
        ...
        if ($form->checkInput()) {
            ...
            if ($extra_checks_ok) {
                ...
                $assignment->setLastScoringUpdateUser($this->user->getId());
                $assignment->update();
                ...
                ilUtil::sendSuccess($this->lng->txt("adn_score_saved"), true);
