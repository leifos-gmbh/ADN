...
class adnCertifiedProfessional extends adnDBBase
{
	...
	protected function propertiesToFields()
	{
		$fields = array("salutation" => array("text", $this->getSalutation()),
			...
			"birthdate" => array("timestamp", $this->getBirthdate()->get(IL_CAL_DATE,
				"", ilTimeZone::UTC)),
		...
		if($date && !$date->isNull())
		{
			$fields["blocked_until"] = array("timestamp", $date->get(IL_CAL_DATE, "",
				ilTimeZone::UTC));
		}
		else
		{
	...
	public function delete()
	{
		...
		if($id)
		{
			...
			if(!$in_use)
			{
				...
				$all = adnCertificate::getAllCertificates(array("cp_professional_id"=>$id),
					true, true);
				if(sizeof($all))
				{
					$in_use = true;
	 ...
	 */
	public static function isUserUnique($a_last_name, $a_first_name, ilDate $a_birthdate,
		$a_id = null, $a_return_id = false)
	{
		global $ilDB;
		...
		$sql = "SELECT id".
			...
			" AND birthdate = ".$ilDB->quote($a_birthdate->get(IL_CAL_DATE, "", ilTimeZone::UTC),
				"timestamp").
