...
core.widget('topic', {
	...
	_create: function() {
		...
		element.addClass('nethelp-topic');
	...
	load: function(url, event, params) {
		...
		if (query) {
			...
			return self.request = $.ajax({
				...
				try {
					idoc.open();
					idoc.write(core.escapeJS(req.responseText));
					idoc.close();
					if (self.trigger('validate', event, $.extend({}, params, { frame: ifr, document: doc }))) {
						t = {
							...
							scripts: doc.find('script[type="_js"]')
								...
								.map(function() {
									...
									return core.unescapeJSNode(core.importNode(this, { content: 'text', native: false }), true);
								})
							...
							styles: $($(idoc.createElement('p')).append(doc.find('style').remove()).html())
								.add(doc.find('link[rel~="stylesheet"]').remove().map(function() {
									return core.importNode(this, { native: false, content: false });
								})),
							links: doc.find('link')
								.remove()
								.map(function() {
									return core.importNode(this, { content: false });
								}),
							meta: meta,
							virtualDoc: true,
							src: loadUrl
						...
						t.body = doc.find('head').html() + doc.find('body').html();
						doc = t;
						ifr.remove();
						ifr = idoc = undefined;
