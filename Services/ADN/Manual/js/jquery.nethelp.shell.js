...
$.each({
	...
	var collProp = props[0],
		...
		createProp = props[2];
	shell[collProp] = [];
	shell[prop] = function(descriptor) {
		var self = this,
			coll = self[collProp],
			i = coll.length,
	...
	start: function(options) {
		...
		*/
		...
		shell.writeBody = function() {
			bodyHtml && document.write(bodyHtml);
		};
		...
		shell.loadSettings(options).pipe(function(settings) {
			...
			doc.open();
			doc.write(core.escapeJS(req.responseText));
			doc.close();
			doc = $(doc);
			...
			//#endregion
			headHtml += core.unescapeJS(doc.find('head').html());
			...
			bodyHtml = core.unescapeJS(doc.find('body').html());
			fr.remove();
		}).done(function() {
			$(proxy(d, 'resolve')); // start when dom is ready
		});
		//#endregion
		return abort ? $.Deferred().reject() : d
