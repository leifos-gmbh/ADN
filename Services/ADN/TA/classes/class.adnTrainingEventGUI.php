...
class adnTrainingEventGUI
{
	...
	protected function listTrainingEvents()
	{
		...
		{
			$ilToolbar->addButton($lng->txt("adn_add_training_event"),
	...
	{
		include_once("./Services/ADN/TA/classes/class.adnTrainingEventTableGUI.php");
		$table = new adnTrainingEventTableGUI($this, "listTrainingEvents", $this->provider_id, false,
			false, !$this->not_overview);
		$table->resetOffset();
		$table->writeFilterToSession();
	...
	function setTabs($a_activate)
	{
		...
		$ilTabs->addTab("current_tr_events",
			$lng->txt("adn_current_tr_events"),
			$ilCtrl->getLinkTarget($this, "listTrainingEvents"));

		$ilCtrl->setParameter($this, "arc", "1");

		$ilTabs->addTab("archived_tr_events",
