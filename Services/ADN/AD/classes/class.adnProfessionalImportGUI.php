...
class adnProfessionalImportGUI
{
	...
	protected function confirmImport()
	{
		...
		if($form->checkInput())
		{
			...
			{
				// init dry run
				$log = $this->importCSV($target, $form->getInput("wmo"), $form->getInput("country"),
					$form->getInput("anon"), $form->getInput("limit"));
	...
	protected function saveImport()
	{
		...
		if($token)
		{
			...
			{
				if($this->importCSV($target, $_REQUEST["wmo"], $_REQUEST["country"],
					$_REQUEST["anon"], $_REQUEST["limit"], false))
				{
					unlink($target);
	...
	{		
		...
		{
			$old_id = adnCertifiedProfessional::isUserUnique($a_pros[$hash]["last_name"],
				$a_pros[$hash]["first_name"],
	 ...
	 */
	protected function importCSV($a_file, $a_default_wmo, $a_default_country, $a_anonymize = false,
		$a_limit = null, $a_dry_run = true)
	{				
