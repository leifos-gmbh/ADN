...
class adnMCQuestionExportGUI
{
	...
	protected function confirmImport()
	{
		...
		if($form->checkInput())
		{
			...
			if(move_uploaded_file($file["tmp_name"], $target))
			{
				...
				$log = $export->processImport($target, in_array("mc", $opts), in_array("case", $opts),
					in_array("obj", $opts), in_array("tgt", $opts), in_array("goods", $opts),
					(bool)$form->getInput("delall"), (bool)$form->getInput("update"), true);
	...
	protected function saveImport()
	{
		...
		if($token)
		{
			...
			{
				$opts = $_REQUEST["opts"];
				if($export->processImport($target, in_array("mc", $opts), in_array("case", $opts),
					in_array("obj", $opts), in_array("tgt", $opts), in_array("goods", $opts),
					$_REQUEST["delall"], $_REQUEST["update"], false))
				{
					unlink($target);
