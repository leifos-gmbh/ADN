...
class adnQuestionImport
{
	 ...
	 * @return bool
	 */
	public function processImport($a_tmp_file, $a_mc, $a_case, $a_objectives = false,
		$a_targets = false, $a_goods = false, $a_delete_all = false,
		$a_update = false, $a_dry_run = true)
	{
		$zip = new ZipArchive;
		if ($zip->open((string)$a_tmp_file))
		{
			 ...
			 if((bool)$a_goods)
			 {
				 $this->importGoodCategories($data->good_categories->good_category, $a_delete_all,
					$a_update, $a_dry_run);
			 ...
			 if((bool)$a_objectives)
			 {
				 $this->importObjectives($data->objectives->objective, $a_delete_all, $a_update,
					$a_dry_run);
				 $this->importSubobjectives($data->subobjectives->subobjective, $a_delete_all,
					$a_update, $a_dry_run);
			 }
			 ...
			 if((bool)$a_targets)
			 {
				 $this->importTargets($data->target_numbers->area, $a_delete_all, $a_update,
					$a_dry_run);
			 }
			 if((bool)$a_mc)
			 {
				$this->importMCQuestions($data->questions_mc->question, $a_delete_all, $a_update,
					$a_dry_run);
			 }
			 if((bool)$a_case)
			 {
				$this->importCaseQuestions($data->questions_case->question, $a_delete_all,
					$a_update, $a_dry_run);
			 }
	 ...
	 * @param bool $a_dry_run
	 */
	protected function importGoodCategories(SimpleXMLElement $data, $a_delete_all,
		$a_update, $a_dry_run)
	{
		$map = array();
		foreach(adnGoodInTransitCategory::getCategoriesSelect() as $id => $caption)
	...
	protected function buildObjectiveMap()
	{
		foreach(adnObjective::getAllObjectives() as $item)
		{
			...
			if($subs)
			{
				...
				{
					$snumber = adnQuestionExport::buildNumber($item["catalog_area"], $item["nr"],
						$sitem["nr"]);
	 ...
	 * @param bool $a_dry_run
	 */
	protected function importObjectives(SimpleXMLElement $data, $a_delete_all,
		$a_update, $a_dry_run)
	{
	 ...
	 * @param bool $a_dry_run
	 */
	protected function importSubobjectives(SimpleXMLElement $data, $a_delete_all, $a_update,
		$a_dry_run)
	{
			...
			if($number)
			{
				...
				else
				{
					...
					if(!$a_dry_run)
					{
						...
						{
							// only if new entry
							$obj_id = adnQuestionExport::buildNumber($number["catalog_area"],
								$number["objective"]);
	 ...
	 * @param bool $a_dry_run
	 */
	protected function importMCQuestions(SimpleXMLElement $data, $a_delete_all, $a_update,
		$a_dry_run)
	{
			...
			if($number)
			{
				...
				else
				{
					...
					else
					{
						...
						$quest->setAnswerA((string)$question->answers->A->text,
							(string)$question->answers->A->image);
						$quest->setAnswerB((string)$question->answers->B->text,
							(string)$question->answers->B->image);
						$quest->setAnswerC((string)$question->answers->C->text,
							(string)$question->answers->C->image);
						$quest->setAnswerD((string)$question->answers->D->text,
							...
							else
							{
								...
								if($number["subobjective"])
								{
									$sobj_id = adnQuestionExport::buildNumber(
										$number["catalog_area"],
										$number["objective"], $number["subobjective"]);
								}
								if($sobj_id)
								{
	 ...
	 * @param bool $a_dry_run
	 */
	protected function importCaseQuestions(SimpleXMLElement $data, $a_delete_all, $a_update,
		$a_dry_run)
	{
		foreach($data as $question)
		{
				...
				else
				{
					...
					if(!$a_dry_run)
					{
						...
						else
						{
							...
							if($number["subobjective"])
							{
								$sobj_id = adnQuestionExport::buildNumber($number["catalog_area"],
									$number["objective"], $number["subobjective"]);
							}
							if($sobj_id)
							{
	...
	protected function parseObjectiveNumber($a_number)
	{
		...
		else if(preg_match("/^([0-9]+) ([0-9]+)$/", trim($a_number), $parts))
		{
			$res["catalog_area"] = (int)$parts[1];
	...
	protected function parseQuestionNumber($a_number)
	{
		...
		// mc
		else if(preg_match("/^([0-9]+) ([0-9]+)\.([0-9]+)\-([0-9]+)$/", trim($a_number), $parts))
		{
			$res["catalog_area"] = (int)$parts[1];
			$res["objective"] = (int)$parts[2];
			$res["subobjective"] = (int)$parts[3];
