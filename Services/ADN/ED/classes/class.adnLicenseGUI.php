...
class adnLicenseGUI
{
	...
	protected function listLicenses()
	{
		...
		if(adnPerm::check(adnPerm::ED, adnPerm::WRITE))
		{
			...
			{
				$ilToolbar->addButton($lng->txt("adn_add_license"),
					$ilCtrl->getLinkTarget($this, $this->getLink("addLicense")));
			}
			else if(!$table->getData())
			{
				$ilToolbar->addButton($lng->txt("adn_add_gas_license"),
					$ilCtrl->getLinkTarget($this, $this->getLink("addLicense")));
			}
		}
	...
	protected function initLicenseForm($a_mode = "edit")
	{
		...
		if($this->type == adnLicense::TYPE_CHEMICALS)
		{
			...
			$goods = adnGoodInTransit::getGoodsSelect(adnGoodInTransit::TYPE_CHEMICALS,
				null, $goods);
			if($goods)
			{
				$specific = new ilCheckboxGroupInputGUI($lng->txt("adn_goods_in_transit"),
					"goods");
	...
	function setTabs()
	{
		...
		$ilTabs->addTab(adnLicense::TYPE_CHEMICALS,
			$lng->txt("adn_licenses_chem"),
			$ilCtrl->getLinkTarget($this, "listLicensesChem"));

		$ilTabs->addTab(adnLicense::TYPE_GAS,
