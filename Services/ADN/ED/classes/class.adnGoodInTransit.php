...
class adnGoodInTransit extends adnDBBase
{
	 ...
	 */
	public static function getGoodsSelect($a_type = null, $a_category = null,
		array $a_old_values = null, $a_force_all = false)
	{
		global $ilDB;
		...
		while($row = $ilDB->fetchAssoc($res))
		{
			$all[$row["id"]] = self::handleName($row["name"]." (".$row["un_nr"].")",
				$row["archived"]);
		}
