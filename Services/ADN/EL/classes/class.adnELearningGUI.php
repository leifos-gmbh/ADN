...
class adnELearningGUI
{
	...
	public function executeCommand()
	{
		...
		switch ($next_class)
		{
			case "adntestgui":
				...
				$test_gui = new adnTestGUI(adnTestGUI::MODE_ELEARNING,
	...
	function showFrontPage()
	{
		...
		$etpl = new ilTemplate("tpl.front_page.html", true, true,
			"Services/ADN/EL");
		...
		foreach (adnSubjectArea::getAllAreas() as $k => $txt)
		{
			...
			$etpl->setVariable("HREF_START_TEST",
		...
		foreach ($letters as $k => $f)
		{
			...
			$etpl->setVariable("HREF_DOWNLOAD",
	...
	function showResult()
	{
		...
		$ilToolbar->addButton($lng->txt("adn_back_to_front_page"),
			$ilCtrl->getLinkTarget($this, "showFrontPage"));
		$ilToolbar->addButton($lng->txt("adn_download_solutions"),
		...
		$res_table = new adnELResultTableGUI($this, "showResult",
			$_SESSION["sheet_questions"], $_SESSION["given_answer"]);
