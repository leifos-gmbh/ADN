...
class adnCertificateTableGUI extends ilTable2GUI
{
	 ...
	 */
	function __construct($a_parent_obj, $a_parent_cmd, $a_cp_id = 0)
	{
		global $ilCtrl, $lng;
	...
	protected function importData()
	{
		...
		$certificates = adnCertificate::getAllCertificates($cert_filter, $include_invalids,
			false, $exclude_valids);
	...
	function initFilter()
	{
		...
		$number = $this->addFilterItemByMetaType("number", self::FILTER_TEXT, false,
			$lng->txt("adn_number"));
		$number->readFromSession();
		$this->filter["number"] = $number->getValue();

		$last_name = $this->addFilterItemByMetaType("last_name", self::FILTER_TEXT, false,
	...
	protected function fillRow($a_set)
	{
		...
		if ($this->cp_id > 0)
		{
			...
			{
				$this->tpl->setVariable("VAL_ARCHIVED_ON",
