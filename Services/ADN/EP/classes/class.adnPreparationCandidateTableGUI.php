...
class adnPreparationCandidateTableGUI extends ilTable2GUI
{
	 ...
	 */
	function __construct($a_parent_obj, $a_parent_cmd, $a_show_professionals = false,
			...
			foreach($candidates as $idx => $item)
			{
				...
				if(!$item["subject_area"])
				{
					if($this->show_professionals || $this->show_prospects)
					{
						...
						// is prospect?
						else if(!in_array($item["id"], $all_valid) && !$this->show_prospects)
						{
							unset($candidates[$idx]);
							continue;
						}
	...
	function initFilter()
	{
		...
		$name = $this->addFilterItemByMetaType("name", self::FILTER_TEXT, false,
			$lng->txt("adn_last_name"));
		$name->readFromSession();
		$this->filter["last_name"] = $name->getValue();

		$first_name = $this->addFilterItemByMetaType("first_name", self::FILTER_TEXT, false,
			$lng->txt("adn_first_name"));
		$first_name->readFromSession();
		$this->filter["first_name"] = $first_name->getValue();

		$birthdate = $this->addFilterItemByMetaType("birthdate", self::FILTER_DATE_RANGE, false,
			$lng->txt("adn_birthdate"));
		$birthdate->readFromSession();
		$this->filter["birthdate"] = $birthdate->getDate();
		
		$types = $this->addFilterItemByMetaType("type", self::FILTER_SELECT, false,
