...
class adnAnswerSheetAssignmentTableGUI extends ilTable2GUI
{
	 ...
	 */
	function __construct($a_parent_obj, $a_parent_cmd, $a_event_id, $a_archived = false)
	{
		global $ilCtrl, $lng;
		...
		if(adnPerm::check(adnPerm::EP, adnPerm::WRITE))
		{
			...
			$this->addMultiCommand("downloadExaminationDocuments",
		...
		if($assignments)
		{
			...
			foreach($assignments as $item)
			{
				...
				$data[] = array("id" => $user->getId(),
					...
					"assigned" => adnAnswerSheetAssignment::getSheetsSelect($user->getId(),
						$this->event->getId())
					);
			}
	...
	protected function fillRow($a_set)
	{
		...
		if(adnPerm::check(adnPerm::EP, adnPerm::WRITE))
		{
			...
			if(!$this->event_done)
			{
				...
				foreach($selects as $type)
				{
					...
					$this->tpl->setVariable("VAL_ASSIGNED_OPTION_TITLE",
					...
					if($this->map["sheets_split"][$type])
					{
						foreach($this->map["sheets_split"][$type] as $id => $caption)
						{
							...
							{
								$this->tpl->setVariable("VAL_ASSIGNED_OPTION_STATUS",
									" selected=\selected\"");
							}
							$this->tpl->parseCurrentBlock();
						}
		...
		if($a_set["assigned"])
		{
			...
			{
				if($mtime = adnReportAnswerSheet::lookupCandidateSheetGenerated($a_set['id'],
					$assigned))
				{
					$this->tpl->setCurrentBlock("generated");
					$this->tpl->setVariable("VAL_GENERATED_NUMBER", $this->map['sheets'][$assigned]);
					$this->tpl->setVariable("VAL_GENERATED_DATE", 
