...
class adnAnswerSheetQuestionTableGUI extends ilTable2GUI
{
	 ...
	 */
	function __construct($a_parent_obj, $a_parent_cmd, adnAnswerSheet $a_sheet, $a_obj_id,
		$a_sobj_id, array $a_question_ids = null)
	{
		global $ilCtrl, $lng;
		...
		if(adnPerm::check(adnPerm::EP, adnPerm::WRITE))
		{
			...
			else
			{
				...
				$this->objective_options = adnObjective::getObjectivesSelect($obj->getCatalogArea(),
					$obj->getType(), true);
			}
			$this->addColumn("", "", 1);
		}
		...
		foreach($ids as $id)
		{
				...
				in_array($id, $this->question_ids)))
			{
				...
				else
				{
					...
					if($valid)
					{
						$correct_answer = adnGoodRelatedAnswer::getAnswerForSheet($this->sheet,
							$question);
						if(!$correct_answer)
						{
							$valid = false;
