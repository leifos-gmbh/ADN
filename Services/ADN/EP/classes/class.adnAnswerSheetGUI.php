...
class adnAnswerSheetGUI
{
	...
	protected function listEvents()
	{
		...
		$table = new adnExaminationEventTableGUI($this, "listEvents",
			adnExaminationEventTableGUI::MODE_SHEET, $this->archived);
	...
	protected function listSheets()
	{
		...
		$ilTabs->addTab("sht", $lng->txt("adn_answer_sheets"),
			$ilCtrl->getLinkTarget($this, "listSheets"));
		$ilTabs->addTab("ass", $lng->txt("adn_assignment_generating"),
		...
		{
			$ilToolbar->addButton($lng->txt("adn_add_mc_answer_sheet"),
			...
			{
				$ilToolbar->addButton($lng->txt("adn_add_case_answer_sheet"),
	...
	protected function listQuestionsForSheet()
	{
		...
		foreach($sheet_data["objectives"] as $objective_number => $objective)
		{
			...
			if(!$read_only && $objective["addable"])
			{
				...
				$mytpl->setVariable("VAL_OBJECTIVE_URL", $ilCtrl->getLinkTarget($this,
					"addQuestionToSheet"));
			...
			if(sizeof($oquestions))
			{
				...
				foreach($oquestions as $question_number => $question)
				{
					...
					$mytpl->setVariable("VAL_QUESTION",
			...
			foreach($sheet_data["subobjectives"] as $subobjective_number => $subobjective)
			{
				...
				if($subobjective["objective_id"] == $objective_number)
				{
					...
					if(!$read_only && $subobjective["addable"])
					{
						...
						$mytpl->setVariable("VAL_SUBOBJECTIVE_URL",
							$ilCtrl->getLinkTarget($this, "addQuestionToSheet"));
						$ilCtrl->setParameter($this, "sobj_id", "");
						$mytpl->setVariable("VAL_SUBOBJECTIVE_ACTION",
		...
		// overall info
		$info = sprintf($lng->txt("adn_sheet_question_status"), sizeof($sheet_data["questions"]),
			$sheet_data["target"]);
		...
		{
			$info .= ", ".sprintf($lng->txt("adn_sheet_question_status_invalid"),
				$invalid_questions);
		}
		...
		else if($sheet_data["target"] < sizeof($sheet_data["questions"]))
		{
	...
	protected function addQuestionToSheet()
	{
		...
		$table = new adnAnswerSheetQuestionTableGUI($this, "addQuestionToSheet", $this->sheet,
			$_REQUEST["obj_id"], $_REQUEST["sobj_id"]);
	...
	protected function listAssignment()
	{
		...
		$table = new adnAnswerSheetAssignmentTableGUI($this, "listAssignment",
			$event_id, $this->archived);
	...
	protected function showSheetSubjectedList(array $a_question_ids)
	{
		...
		include_once("./Services/ADN/EP/classes/class.adnAnswerSheetQuestionTableGUI.php");
		$table = new adnAnswerSheetQuestionTableGUI($this, "addQuestionToSheet", $this->sheet,
			$_REQUEST["obj_id"], $_REQUEST["sobj_id"], $a_question_ids);
