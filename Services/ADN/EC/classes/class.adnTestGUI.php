...
class adnTestGUI
{
	...
	public function __construct($a_mode = self::MODE_ONLINE, $a_questions = null)
	{
		...
		else if ($a_mode == self::MODE_ELEARNING)
		{
			$cnt = 1;
	 ...
	 */
	function determineCandidateAndExamination()
	{
		global $lng;
	...
	function showQuestionList()
	{
		...
		$table = new adnTestQuestionListTableGUI($this, "showQuestionList",
			$this->questions, $this->cand_sheet_id);
		...
		if (is_array($this->questions))
		{
			foreach ($this->questions as $k => $q)
			{
				if ($q["q_id"] == $q_id)
				{
					...
					else if ($k == count($this->questions) - 1)
					{
						$last = true;
					}
				}
	...
	protected function saveAnswer()
	{
		if ((int) $_POST["q_id"] > 0)
		{
			...
			{
				adnTest::saveAnswer($this->cand_sheet_id, (int) $_POST["q_id"],
	 ...
	 */
	function finishTestConfirmation()
	{
		...
		$ilCtrl->setParameter($this, "q_id", $_GET["q_id"]);
		$ilToolbar->addButton($lng->txt("yes"),
			$ilCtrl->getLinkTarget($this, "finishTest"));
		$ilToolbar->addButton($lng->txt("no"),
